...
def analyze_market():
    ...
    # --- 訊息組裝 (Strict Top 10) ---
    # 這裡進行強制切片，確保最終清單長度 <= 10
    final_strong = sorted(strong_list, key=lambda x: x['pct'], reverse=True)[:10]
    final_ready = sorted(ready_list, key=lambda x: x['vol_ratio'], reverse=True)[:10]

    # 標題顯示總數，讓您知道還有更多
    msg = "【📊 AI 全方位掃描】\n"
    msg += f"🔥 強勢: 共{len(strong_list)}檔 (精選前{len(final_strong)}檔)\n"
    msg += f"📦 蓄勢: 共{len(ready_list)}檔 (精選前{len(final_ready)}檔)\n"
    msg += "="*16 + "\n"

    if not final_strong and not final_ready:
        msg += "今日無明確訊號，建議觀望。"
    else:
        if final_strong:
            msg += f"🚀 強勢股 (Top {len(final_strong)}):\n"
            for s in final_strong:
        ...
        if final_ready:
            msg += f"\n📦 盤整蓄勢 (Top {len(final_ready)}):\n"
            for s in final_ready:
                msg += f"👀 {s['code']} {s['name']} {s['pred']}\n"
                msg += f"💰 {s['price']} (整理)\n"
